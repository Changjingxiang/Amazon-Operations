...
function installHandlers(){
  ...
  handle('pick-report','main',async()=>{
    ...
    return{id:reportFile.id,filename:reportFile.filename,headers:parsed.headers,mapping:core.suggestMapping(parsed.headers),hint:core.reportHint(parsed.headers),reader:parsed.reader,sample:parsed.table.slice(0,3),count:parsed.table.length,asOf:reportFile.asOf};
  });
