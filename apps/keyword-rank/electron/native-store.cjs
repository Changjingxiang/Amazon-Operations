...
function migrateWorkbook(toolRoot, exporterPath, cachePath) {
  ...
  const configs = data.models.map(({ modelName, parentAsin, site, countryCode, dashboardSheet, historySheet, naturalMatrixSheet, spMatrixSheet, abaMonthlySheet, order }) => ({
    modelName, parentAsin, site: countryLabel(countryCode || site), countryCode: normalizeCountryCode(countryCode || site), dashboardSheet, historySheet, naturalMatrixSheet, spMatrixSheet, abaMonthlySheet, order,
  }));
  const histories = {};
...
function setModelCountry(toolRoot, exporterPath, cachePath, payload) {
  const store = ensureStore(toolRoot, exporterPath, cachePath);
  const asin = text(payload.parentAsin).toUpperCase();
  const config = store.configs.find((item) => (asin && item.parentAsin === asin) || (payload.modelName && item.modelName === text(payload.modelName)));
  if (!config) throw new Error('找不到对应产品型号。');
  const countryCode = normalizeCountryCode(payload.countryCode || payload.site || 'CA');
...
function deleteModel(toolRoot, exporterPath, cachePath, payload) {
  const store = ensureStore(toolRoot, exporterPath, cachePath);
  const asin = text(payload.parentAsin).toUpperCase();
  ...
  for (const [fileName, info] of Object.entries(store.importedFiles)) {
    if (info && info.parentAsin === removed.parentAsin) delete store.importedFiles[fileName];
  }
  writeStore(toolRoot, store);
  return { ok: true, output: `已删除型号“${removed.modelName}”。`, data: readData(toolRoot, exporterPath, cachePath) };
}
...
function importReports(toolRoot, exporterPath, cachePath, mode = 'normal') {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (strict) {
        failed++;
        errors.push(`${file.name}：${error.message || '文件格式不支持'}`);
      } else {
