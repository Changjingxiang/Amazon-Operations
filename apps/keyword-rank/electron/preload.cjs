...
contextBridge.exposeInMainWorld('keywordTracker', {
  ...
  renameModel: (payload) => ipcRenderer.invoke('tracker:rename-model', payload),
  changeModelAsin: (payload) => ipcRenderer.invoke('tracker:change-model-asin', payload),
  releaseModelAlias: (payload) => ipcRenderer.invoke('tracker:release-model-alias', payload),
