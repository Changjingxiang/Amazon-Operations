        ...
        for y in range(h):
            for x in range(w):
                ...
                stats.append({
                    ...
                    'cx': sum_x / size, 'cy': sum_y / size,
                    'bbox': (min_x, min_y, max_x + 1, max_y + 1),
                })
        center_x, center_y = w / 2.0, h / 2.0
        candidates = [s for s in stats if abs(s['cx'] - center_x) < 0.34 * w and abs(s['cy'] - center_y) < 0.34 * h]
        main = max(candidates or stats, key=lambda s: s['size'])
        mx0, my0, mx1, my1 = main['bbox']
        link = 0.05 * max(h, w)
        keep = {main['label']}
        for s in stats:
            if s['label'] == main['label'] or s['size'] < 0.02 * main['size']:
                continue
            x0, y0, x1, y1 = s['bbox']
            dx = max(0, x0 - mx1, mx0 - x1)
            dy = max(0, y0 - my1, my0 - y1)
            if dx <= link and dy <= link:
                keep.add(s['label'])
        out = alpha.copy()
...
def main():
    ...
    ap.add_argument("--mode", choices=["floodfill", "rembg", "alpha"], default="floodfill")
    ...
    source = Image.open(a.sheet).convert("RGBA")
    im = source if a.mode == "alpha" else source.convert("RGB")
    W, H = im.size
    g = a.grid
    total_cells = g * g
    ...
    cw, ch = W // g, H // g
    # 透明母版不需要外扩；外扩会把相邻衣物的边缘带进切图。
    pad = 0 if a.mode == "alpha" else int(0.06 * min(cw, ch))
    session = None
    if a.mode == "rembg":
    ...
    for r in range(g):
        for c in range(g):
            ...
            rgb = np.array(cell.convert("RGB")).astype(int)
            if a.mode == "floodfill":
