...
export default function MatrixPeriodSelect({ dates, filter = {}, onChange }) {
  ...
  return <div className="matrix-period-select" ref={root}>
    ...
    <button ref={trigger} type="button" className="matrix-month-trigger" aria-expanded={open} onClick={() => setOpen(current => !current)}>月份：{selected.length ? selected.map(m => Number(m.slice(5)) + '月').join('、') : '请选择'}<ChevronDown size={14} /></button>
    {open && <div className="matrix-month-menu is-open">
