...
export default function MatrixView({ model, metric, selectedDate, onToggleWatch, onSetAnnotation }) {
  ...
  const annotationField = metric === 'sp' ? 'spAnnotations' : null;
  ...
  const beginAnnotation = (row, date, existing = '') => {
    if (metric !== 'sp' || !date) return;
    setEditing({ keyword: row.keyword, date, draft: existing });
  };
  const commitAnnotation = async () => {
    if (!editing) return;
    const payload = { keyword: editing.keyword, date: editing.date, text: editing.draft.trim() };
  ...
  return (
    <section className="matrix-panel">
      <div className="matrix-note matrix-group-note"><span>{metric === 'natural' ? '自然矩阵' : 'SP矩阵'}：可按年份、月份收放</span><span>关键词为行、日期为列；0 表示未上榜。{metric === 'sp' ? '点击 SP 排名单元格添加标注，黑底白字表示已标注。' : ''}</span><span><b className="legend-up">红色</b>=排名上升　<b className="legend-down">绿色</b>=排名下降　<b className="legend-none">灰色</b>=未上榜</span></div>
      <div className="matrix-scroll">
        <table className={`matrix-table matrix-group-table matrix-layout-transition ${layoutAnimating ? 'is-animating' : ''}`}>
          <colgroup>
          ...
          <thead>
            ...
            <tr><th className="sticky-col star-col" style={widthStyle('star')}>关注{resizeHandle('star', '关注')}</th><th className="sticky-col keyword-col" style={widthStyle('keyword')}>关键词{resizeHandle('keyword', '关键词')}</th><th className="sticky-col translation-col" style={widthStyle('translation')}>翻译{resizeHandle('translation', '翻译')}</th>{columns.map((column) => <th key={column.key} style={widthStyle('date')} className={`${column.type !== 'date' ? 'matrix-placeholder-head' : ''} ${column.date === selectedDate ? 'selected-date' : ''}`}>{column.type === 'date' ? shortDate(column.date) : ''}{resizeHandle('date', '日期')}</th>)}</tr>
          </thead>
          <tbody>{model.matrixRows.map((row) => { const values = row[valueField] || []; const annotations = annotationField ? (row[annotationField] || []) : []; return <tr key={row.keyword} className={row.watched ? 'watched-row' : ''}>
            ...
            {columns.map((column) => { if (column.type !== 'date') return <td key={`${row.keyword}-${column.key}`} className="matrix-placeholder" aria-label="折叠分组" />; const index = model.dates.indexOf(column.date); const value = values[index]; const visibleValue = displayRank(value); const annotation = annotations[index] || ''; const previous = index ? values[index - 1] : null; const isEditing = editing?.keyword === row.keyword && editing.date === column.date; return <td key={`${row.keyword}-${column.date}`} className={`${rankClass(value, previous)} ${metric === 'sp' ? 'sp-annotation-cell' : ''} ${annotation ? 'sp-annotated-cell' : ''} ${column.date === selectedDate ? 'selected-date' : ''}`} data-rank={visibleValue} title={annotation ? `标注：${annotation}` : metric === 'sp' ? '点击添加标注；双击可再次编辑' : undefined} onClick={() => beginAnnotation(row, column.date, annotation)} onDoubleClick={() => beginAnnotation(row, column.date, annotation)}>{isEditing ? <input className="cell-annotation-input" autoFocus value={editing.draft} onChange={(event) => setEditing((current) => current ? { ...current, draft: event.target.value } : current)} onClick={(event) => event.stopPropagation()} onDoubleClick={(event) => event.stopPropagation()} onBlur={commitAnnotation} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); commitAnnotation(); } if (event.key === 'Escape') { event.preventDefault(); setEditing(null); } }} aria-label={`编辑${row.keyword} ${column.date}标注`} /> : <span className="sp-rank-value">{visibleValue}</span>}</td>; })}
          </tr>; })}</tbody>
        </table>
      </div>
    </section>
