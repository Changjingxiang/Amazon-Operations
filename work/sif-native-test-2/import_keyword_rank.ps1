...
try {
    ...
    $watchSync = Sync-WatchEntriesFromViews $tracker $configs
    ...
    $watchBackfillRequests = @(Get-WatchBackfillRequests $tracker $configs $watches)
    ...
    for ($fileIndex = 0; $fileIndex -lt $pendingFiles.Count; $fileIndex++) {
        ...
        try {
            ...
            $config = $configs | Where-Object { $_.ParentAsin -eq $report.ParentAsin } | Select-Object -First 1
