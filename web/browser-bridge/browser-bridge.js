(function () {
  ...
  function pageAsin() {
    return text(document.querySelector('.topbar p')?.textContent?.match(/B0[A-Z0-9]{8}/i)?.[0]).toUpperCase();
  }
  ...
  async function runWebExtensionBatch(items, options = {}) {
    ...
    const normalizedItems = (Array.isArray(items) ? items : []).map((item) => ({
      ...
      kind: text(item?.kind || item?.type || 'own').toLocaleLowerCase('en-US') === 'competitor' ? 'competitor' : 'own',
    ...
    try {
      const response = await requestWebExtension('START_WEB_BATCH', {
        ...
        // Phased imports pass concurrency=1 so the extension cannot start a
        // later ASIN until the previous one has completed.  The default keeps
        // the legacy small-batch behavior for callers that do not opt in.
        concurrency: Math.max(1, Math.min(Number(options.concurrency) || Math.min(3, normalizedItems.length), normalizedItems.length)),
      }, 5000);
  ...
  function normalizeAnnotations(value) {
    ...
    return value
      ...
      .map((item) => ({
        modelName: text(item.modelName),
        parentAsin: text(item.parentAsin).toUpperCase(),
  ...
  // Competitors intentionally live beside, rather than inside, the primary
  // product configuration.  This keeps them out of the product sidebar while
  // still letting the normal report importer match their ASINs and build the
  // same dashboard/matrix-shaped data model for the comparison drawer.
  function normalizeCompetitor(value, index = 0) {
    const competitor = value && typeof value === 'object' ? value : {};
    const parentAsin = text(competitor.parentAsin || competitor.asin).toUpperCase();
    const ownerParentAsin = text(
      competitor.ownerParentAsin || competitor.ownerAsin || competitor.productParentAsin,
    ).toUpperCase();
    const name = text(competitor.competitorName || competitor.modelName || competitor.name)
      || `竞品 ${index + 1}`;
    const id = text(competitor.id || competitor.competitorId)
      || `${ownerParentAsin}:${parentAsin}:${index}`;
    const historySheet = text(competitor.historySheet)
      || `competitor_${parentAsin || `unknown_${index}`}_历史`;
    const countryCode = normalizeCountryCode(competitor.countryCode || competitor.site);
    return {
      ...competitor,
      kind: 'competitor',
      id,
      ...
      historySheet,
      countryCode,
      site: text(competitor.site) || countryLabel(countryCode),
      order: Number.isFinite(Number(competitor.order)) ? Number(competitor.order) : index,
    };
  }
  ...
  function normalizeStore(value) {
    ...
    return {
      ...
      competitors: Array.isArray(store.competitors)
        ? store.competitors.map((item, index) => normalizeCompetitor(item, index))
        : [],
  ...
  async function readData() {
    ...
    store.configs.forEach((config, index) => {
      modelByAsin.set(config.parentAsin, ownModels[index]);
      (config.legacyParentAsins || []).forEach((asin) => modelByAsin.set(asin, ownModels[index]));
    });
    const competitors = store.competitors.map((config) => {
      const owner = modelByAsin.get(config.ownerParentAsin);
      return {
        ...buildModel(config, store.histories[config.historySheet] || [], store.watches, store.annotations, store.abaMonthly),
        kind: 'competitor',
        competitorId: config.id,
        competitorName: config.competitorName || config.modelName,
    ...
    // The React detail components already accept any model-shaped object.  We
    // append competitors to the same model list so selecting a nested item can
    // use the exact existing product page, tabs and interactions.  The web
    // enhancement hides these appended rows in the sidebar/settings and
    // renders them under their owning product instead.
  ...
  async function replaceWatches(payload) {
    ...
    const belongs = (item) => item?.parentAsin
      ? configAsins.has(text(item.parentAsin).toUpperCase())
      : item?.modelName === config.modelName;
    const others = store.watches.filter((item) => !belongs(item));
    const next = (Array.isArray(payload.items) ? payload.items : [])
      .map((item, index) => ({ parentAsin: config.parentAsin, modelName: config.modelName, keyword: text(item.keyword), note: text(item.note), enabled: true, order: index }))
  ...
  async function addCompetitor(payload = {}) {
    const store = await ensureStore();
    const ownerParentAsin = text(payload.ownerParentAsin || payload.ownerAsin).toUpperCase();
    const ownerModelName = text(payload.ownerModelName || payload.ownerModel);
    const owner = store.configs.find((item) =>
      (ownerParentAsin && (item.parentAsin === ownerParentAsin || (item.legacyParentAsins || []).includes(ownerParentAsin)))
      || (ownerModelName && item.modelName === ownerModelName));
    ...
    const competitorAsins = store.competitors.flatMap((item) => [item.parentAsin, ...(item.legacyParentAsins || [])]);
    if ([...ownAsins, ...competitorAsins].some((asin) => text(asin).toUpperCase() === parentAsin)) {
      throw new Error(`父体 ASIN ${parentAsin} 已存在，不能重复添加。`);
    }
    const id = `cmp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const countryCode = normalizeCountryCode(payload.countryCode || payload.site || owner.countryCode || owner.site);
    const competitor = normalizeCompetitor({
      kind: 'competitor',
      id,
      competitorId: id,
      competitorName,
      modelName: competitorName,
      parentAsin,
      ownerParentAsin: owner.parentAsin,
      ownerModelName: owner.modelName,
      countryCode,
      site: countryLabel(countryCode),
      historySheet: `competitor_${parentAsin}_${id.slice(-6)}_历史`,
      order: store.competitors.filter((item) => item.ownerParentAsin === owner.parentAsin).length,
    }, store.competitors.length);
    store.competitors.push(competitor);
    store.histories[competitor.historySheet] = [];
    await writeStore(store);
    return result(`已为“${owner.modelName}”添加竞品“${competitorName}”。导入报表时会按 ASIN 自动匹配。`);
  }
  ...
  async function changeModelAsin(payload) {
    ...
    const competitorConflict = store.competitors.find((item) =>
      item.parentAsin === nextAsin || (item.legacyParentAsins || []).includes(nextAsin));
    if (conflict) throw new Error(`父体 ASIN ${nextAsin} 已被“${conflict.modelName}”使用。`);
    if (competitorConflict) throw new Error(`父体 ASIN ${nextAsin} 已被竞品“${competitorConflict.competitorName || competitorConflict.modelName}”使用。`);
    ...
    store.annotations.forEach((annotation) => {
      if (annotation && aliases.has(text(annotation.parentAsin).toUpperCase())) {
  ...
  async function deleteModel(payload) {
    ...
    store.watches = store.watches.filter((item) => item.parentAsin
  ...
  async function deleteCompetitor(payload = {}) {
    const store = await ensureStore();
    const id = text(payload.competitorId || payload.id);
    const asin = text(payload.parentAsin || payload.asin).toUpperCase();
    ...
    store.competitors.forEach((item) => {
      if (item.ownerParentAsin === removed.ownerParentAsin) item.order = ownerOrder++;
    });
    delete store.histories[removed.historySheet];
    const removedAsins = new Set([removed.parentAsin, ...(removed.legacyParentAsins || [])]);
    store.watches = store.watches.filter((item) => item.parentAsin
  ...
  async function importReports(mode, providedFiles = null) {
    ...
    store.configs.forEach((config) => {
      configsByAsin.set(config.parentAsin.toUpperCase(), { config, kind: 'model' });
      (config.legacyParentAsins || []).forEach((asin) => {
        if (!configsByAsin.has(asin.toUpperCase())) configsByAsin.set(asin.toUpperCase(), { config, kind: 'model' });
      });
    });
    store.competitors.forEach((competitor) => {
      if (competitor.parentAsin && !configsByAsin.has(competitor.parentAsin.toUpperCase())) {
        configsByAsin.set(competitor.parentAsin.toUpperCase(), { config: competitor, kind: 'competitor' });
      }
      (competitor.legacyParentAsins || []).forEach((asin) => {
        if (!configsByAsin.has(asin.toUpperCase())) configsByAsin.set(asin.toUpperCase(), { config: competitor, kind: 'competitor' });
      });
    });
    let imported = 0;
    let skipped = 0;
    ...
    for (const file of [...files].sort((a, b) => a.lastModified - b.lastModified)) {
      ...
      try {
        const report = await sourceReport(file);
        const target = configsByAsin.get(report.parentAsin);
        if (!target) {
          skipped++;
        ...
        store.histories[config.historySheet] = previous
          ...
          .concat(report.records.map((item) => ({
            ...item,
            modelName: config.modelName,
            parentAsin: config.parentAsin,
            dataOwner: target.kind,
            competitorId: target.kind === 'competitor' ? config.id : undefined,
          })));
  ...
  async function runSifImport(payload = {}) {
    ...
    const store = await ensureStore();
    const config = [...store.configs, ...store.competitors].find((item) =>
      item.parentAsin === asin || (item.legacyParentAsins || []).includes(asin));
  ...
  async function startSifBatchImport(payload = {}) {
    ...
    const ownModels = data.ownModels || (data.models || []).filter((model) => model.kind !== 'competitor');
    const ownAsins = new Set(ownModels.map((model) => text(model.parentAsin).toUpperCase()).filter(Boolean));
    const competitorAsins = new Set((data.competitors || []).map((model) => text(model.parentAsin).toUpperCase()).filter(Boolean));
    const inputItems = Array.isArray(payload.items) && payload.items.length
      ? payload.items
      : [
        ...ownModels.map((model) => ({
        ...
        ...(data.competitors || []).map((competitor) => ({
          ...
          countryCode: competitor.countryCode || 'CA',
    ...
    for (const item of inputItems) {
      const asin = text(item?.asin || item?.parentAsin).toUpperCase();
      if (!asin || seen.has(asin)) continue;
      seen.add(asin);
      const explicitKind = text(item?.kind || item?.type).toLocaleLowerCase('en-US');
      const kind = explicitKind === 'competitor' || competitorAsins.has(asin)
        ? 'competitor'
        : ownAsins.has(asin) || explicitKind !== 'competitor' ? 'own' : 'competitor';
      uniqueItems.push({ ...item, asin, kind });
    }
    const ownItems = uniqueItems.filter((item) => item.kind === 'own');
    const competitorItems = uniqueItems.filter((item) => item.kind === 'competitor');
    if (!uniqueItems.length) throw new Error('当前没有已登记产品或竞品。');
    if (!(await pingWebExtension())) {
      throw new Error('未检测到 SIF 自动导入扩展。请先加载扩展，并在扩展详情中打开“允许访问文件网址”。');
    }
    // Keep the two populations in separate extension jobs.  The first job
    // must fully settle (including failed/cancelled tasks) before the second
    // job is even submitted, so a competitor can never start ahead of an own
    // product.  A concurrency of one preserves the requested deterministic
    // A -> B -> C ordering inside each phase as well.
    const runPhase = async (items, label) => {
      if (!items.length) return { ok: true, output: `${label}：无任务。` };
      return runWebExtensionBatch(items, { concurrency: 1, phaseLabel: label });
    };
  ...
  async function importBackup() {
    ...
    if (window.keywordAI?.kind === 'electron') {
      ...
      await enqueuePersistence(() => persistMainAndBackup(next, text(current.storageRevision || current.updatedAt) || null, current));
      location.reload();
      return;
    }
